...
def main():
    ...
    * :ref:`中文API <lif_fc_mnist.main-cn>`
    ...
    net = nn.Sequential(
        nn.Flatten(),
        nn.Linear(28 * 28, 1200,bias=False),
        neuron.LIFNode(tau=tau,v_reset=0.,v_threshold=0.7),
    ...
    with torch.no_grad():
        ...
        for img, label in test_data_loader:
            ...
            for t in range(net.T):
                out_spikes += encoder(img).squeeze(64,28,28)
            # encoder(img)的 尺 寸 是 ``[1, 128, 28, 28]``， 同 样 使 用 ``squeeze()``␣ , →变 换 尺 寸 为 ``[128, 28, 28]``
            if t == 0 or t == net.T - 1:
                out_spikes_c = out_spikes.clone()
            for i in range(out_spikes_c.shape[0]):
                if out_spikes_c[i].max().item() > out_spikes_c[i].min().item():
            # 对 每 个feature map做 归 一 化 ， 使 显 示 更 清 晰
                    out_spikes_c[i] = (out_spikes_c[i] - out_spikes_c[i].
